...
class BookController extends Controller
{
    ...
    public function __construct(){
        $this->selectedFields = ['id', 'number','type_id', 'active', 'title', 'objective', 'year','created_at','updated_at','created_by','updated_by'] ;
    }
    ...
    public function structure(Request $request){
        if (($user = $request->user()) !== null) {
            $Book = RecordModel::select(['id','title','objective','number','year','pdfs'])->where('id',$request->id)->first();
            if ( $Book !== null ) {
                return response()->json([
                    'records' => [
                        'Book' => $Book ,
                        'structure' => $Book->getStructure()
                    ],
                    'message' => __("crud.read.success")
                ], 200);
            }
